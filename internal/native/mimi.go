...
var ErrMimiEncoderNotImplemented = fmt.Errorf("native mimi encoder is not implemented")
...
func loadMimiDecoderTransformer(vb *VarBuilder, cfg MimiConfig) (*mimiDecoderTransformer, error) {
	...
	if len(layers) == 0 {
		return nil, fmt.Errorf("native: no mimi decoder transformer layers found")
	}
...
func (m *MimiModel) QuantizerProject(latentBCT *tensor.Tensor) (*tensor.Tensor, error) {
	if m == nil || m.quantizerOutProj == nil {
		return nil, fmt.Errorf("native: mimi model is not initialized")
	}
...
func (m *MimiModel) DecodeFromLatent(latent *tensor.Tensor) (*tensor.Tensor, error) {
	if m == nil {
		return nil, fmt.Errorf("native: mimi model is nil")
	}
	var err error
	x := latent
