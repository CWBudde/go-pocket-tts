...
func (e *Engine) LatentToMimi(ctx context.Context, latent *Tensor) (*Tensor, error) {
	...
	if !ok {
		return nil, fmt.Errorf("latent_to_mimi graph not found in manifest")
	}
	...
	if !ok {
		return nil, fmt.Errorf("latent_to_mimi: missing 'mimi_latent' in output")
	}
...
func (e *Engine) MimiDecode(ctx context.Context, mimiLatent *Tensor) ([]float32, error) {
	...
	if !ok {
		return nil, fmt.Errorf("mimi_decoder graph not found in manifest")
	}
	...
	if !ok {
		return nil, fmt.Errorf("mimi_decoder: missing 'audio' in output")
	}
